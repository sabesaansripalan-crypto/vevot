<?php
$pageTitle = "Women's Collection — VelvetVogue";
...
$where  = ["p.category = 'women'", "p.id = (SELECT MIN(p2.id) FROM products p2 WHERE p2.name = p.name AND p2.category = p.category)"];
$params = [];
...
$subs = array_filter($db->query("SELECT DISTINCT subcategory FROM products WHERE category='women' ORDER BY subcategory")->fetchAll(PDO::FETCH_COLUMN), fn($s) => strlen(trim($s)) > 1);
...
$availableColors = $db->query("SELECT DISTINCT color FROM products WHERE category='women' AND color IS NOT NULL AND color != '' ORDER BY color")->fetchAll(PDO::FETCH_COLUMN);
?>
...
function applyFilter() {
    ...
    let url = '/velvet vogue/women.php?';
    if (sub)   url += `sub=${sub}&`;
